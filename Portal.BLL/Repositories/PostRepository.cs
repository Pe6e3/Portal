...
public class PostRepository : GenericRepositoryAsync<Post>, IPostRepository
{
    ...
    public async Task<Post> GetPostByIdAsync(int id) => await db.Posts.FirstOrDefaultAsync(x => x.Id == id);
    public async Task<Post> GetPostByIdAsync(int id, string include1) => await db.Posts.Include(include1).FirstOrDefaultAsync(x => x.Id == id);
    public async Task<Post> GetPostByIdAsync(int id, string include1, string include2) => await db.Posts.Include(include1).Include(include2).FirstOrDefaultAsync(x => x.Id == id);
